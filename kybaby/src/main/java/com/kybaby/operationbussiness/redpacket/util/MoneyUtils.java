...
public class MoneyUtils {
	...
	public static String buildRandom() {
		...
		System.out.println("随机字符串===" + (int) ((random * num)) + strTime);
	...
	public static String createSign(Map<String, Object> map) {
		...
		sb.append("key=" + RedPacketConstant.partnerkey);
		 
		System.out.println("生成字符串===="+sb);
	...
	public static String getOrderNo() {
		String order = RedPacketConstant.partner
	...
	public static String doSendMoney(String url, String data) throws Exception {
		KeyStore keyStore  = KeyStore.getInstance("PKCS12");
		  FileInputStream instream = new FileInputStream(new File(RedPacketConstant.KEYSTORE_FILE));//P12文件目录
//		InputStream instream = MoneyUtils.class.getResourceAsStream("/apiclient_cert.p12");
        try {
            keyStore.load(instream, RedPacketConstant.KEYSTORE_PASSWORD.toCharArray());//这里写密码..默认是你的MCHID
        } finally {
            instream.close();
        }
        // Trust own CA and all self-signed certs
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, RedPacketConstant.KEYSTORE_PASSWORD.toCharArray())//这里也是写密码的
                .build();
