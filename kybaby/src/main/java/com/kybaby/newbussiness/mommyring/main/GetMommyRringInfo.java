...
public class GetMommyRringInfo extends MommyRingAction {
	...
	public String execute(){
		...
		if(action.equals("allCategory")){
			allRingCategory=mommyTheTypeInfoBo.getAllRingCategory();
			Object[] remarkCategory=mommyTheTypeInfoBo.getRemarkRingCategory();
			...
			MommyTheTypeInfo someType=mommyTheTypeInfoBo.getSomeCategoryInstanceById(categoryId);
			if(someType!=null&&someType.getIsAvailable().equals("Y")&&someType.getIsMark().equals("Y")){
				...
				if(userId>0){//用户登录，显示订阅的医生圈,如果设置了允许推荐医生圈的话，就显示允许的医生圈，否则就不显示允许的医生圈
					List<MommyRingType> ringTypeList=mommyRingTypeBo.getSomeUserMommyring(userId);
					someRingCategory=this.someRingTypeListManage(ringTypeList);
				}
				//下面获取推荐的医生圈，
				//没登录医生的推荐医生圈包括系统中推荐的医生圈,
				//登录医生的推荐医生圈包括系统中推荐的，同时用户还没有订阅的医生圈
				List<MommyRingType> recommendDoctorRingTypeList=this.getRecommendRingType(userId);
				...
				List<MommyRingType> ringTypeList=mommyRingTypeBo.getSomeCategoryMommyRing(categoryId);
				someRingCategory=this.someRingTypeListManage(ringTypeList);
			...
			MommyRingType someRingType=mommyRingTypeBo.getSomeMommyRingTypeInstance(ringTypeId);
			if(someRingType!=null){
				...
				mommyRingTypeBo.updateSomeMommyRingType(someRingType);
			}
			List<MommyPostInfo> postInfoList=mommyPostInfoBo.getSomeCategoryPostInfo(ringTypeId);
			...
			MommyPostInfo postInfo=mommyPostInfoBo.getPostInfoInstanceById(postInfoId);
			if(postInfo!=null){
				postInfoLabelNameList=mommyRingLabelBo.getSomePostInfoRingLabelNameList(postInfo.getId());
				loginDoctorId=SessionManage.checkSomeSession("userId");
				...
				if(createPerson.startsWith(startStr)){
					...
					String adminName=mommyPostInfoBo.getAdminNameById(adminId);
					if(adminName==null){
						adminName="";
					}
					...
					DoctorInfo doctorInfo=doctorInfoBo.getDoctorInfoByDoctorId(Long.valueOf(createPerson));
					if(doctorInfo!=null){
				...
				List<MommyPostReply> postReplyList=mommyPostReplyBo.getSomePostInfoReply(postInfoId);
				...
				mommyPostInfoBo.updatePostInfoInstance(postInfo);
				mes="操作成功";
				return "success";
			}
			mes="刷新";
			...
			List<MommyPostInfo> postInfoList=mommyPostInfoBo.getSomeCategoryPostInfo(doctorRingId);
			...
			List<MommyRingLabel> doctorRingLabelList=mommyRingLabelBo.getSomeCategoryRingLabelList(ringCategory);
			someCategoryRingLabelObject=this.ringLabelHandle(doctorRingLabelList);
			mes="操作成功";
			return "success";
		}else if(action.equals("mySendPostInfo")){
			loginDoctorId=SessionManage.checkSomeSession("userId");
			List<MommyPostInfo> postInfoList=mommyPostInfoBo.getMySendPostInfoList(loginDoctorId);
			mySendSomePostInfo=this.getSomePostInfoList(postInfoList);
			mes="操作成功";
			return "success";
		}else if(action.equals("myReplyPostInfo")){
			loginDoctorId=SessionManage.checkSomeSession("userId");
			List<MommyPostInfo> postInfoList=mommyPostInfoBo.getMyReplyPostInfoList(loginDoctorId);
	...
	private String getSomeRingTypeMar(long ringTypeId){
		...
		long doctorId=SessionManage.checkSomeSession("userId");
		MommySubscribeUser someSubscribeUser=mommySubscribeUserBo.getSomeUserSubscribeUser(doctorId, ringTypeId);
		MommyRingType someRingType=mommyRingTypeBo.getSomeMommyRingTypeInstance(ringTypeId);
	 ...
	 */
	private List<Object[]> someRingTypeListManage(List<MommyRingType> ringTypeList){
		...
		if(ringTypeList!=null){
			...
			for(int listLength=0;listLength<ringTypeList.size();listLength++){
				...
				MommyRingType someRingType=ringTypeList.get(listLength);
	 ...
	 * @return
	 */
	private List<MommyRingType> getRecommendRingType(long userId){
		List<MommyRingType> ringTypeList=new ArrayList<MommyRingType>();
		if(this.isRecommend()){//能够推荐
			if(userId>0){//医生登录的情况,返回系统推荐的，同时医生没有订阅的
				ringTypeList=mommyRingTypeBo.getSomeMommyRingType(userId);
			}else{//医生未登录的情况下，返回系统中推荐的医生圈
				ringTypeList=mommyRingTypeBo.getOsRecommendRingType();
			}
		}
		return ringTypeList;
	}
	 ...
	 */
	private List<Object[]> getSomePostInfoList(List<MommyPostInfo> postInfoList){
		List<Object[]> somePostInfoList=new ArrayList<Object[]>();
		if(postInfoList!=null){
			Object[] object=null;
			for(int listSizeI=0;listSizeI<postInfoList.size();listSizeI++){
				object=new String[6];
				MommyPostInfo postInfo=postInfoList.get(listSizeI);
				...
				if(doctorId.startsWith(startStr)){
					...
					DoctorInfo doctor=doctorInfoBo.getDoctorInfoByDoctorId(Long.valueOf(doctorId));
					if(doctor!=null){
	...
	private List<List<Object[]>> getSomePostInfoLabelLinkes(List<MommyPostInfo> postInfoList){
		...
		if(postInfoList!=null){
			...
			for(int postInfoSize=0;postInfoSize<postInfoList.size();postInfoSize++){
				MommyPostInfo postInfoInstance=postInfoList.get(postInfoSize);
				long postInfoId=postInfoInstance.getId();
				returnList=mommyRingLabelBo.getSomePostInfoRingLabelIdAndName(postInfoId);
	...
	private List<Object[]> getSomePostInfoReplyInfo(List<MommyPostReply> postReplyList){
		...
		if(postReplyList!=null){
			...
			for(int replySize=0;replySize<postReplyList.size();replySize++){
				object=new String[9];
				MommyPostReply someReply=postReplyList.get(replySize);
				...
				MommyPostReply postReplyInstance=null;
				if(replyId!=0){//replyId=0表示帖子不是回复之前帖子的ID
					postReplyInstance=mommyPostReplyBo.getPostReplyInstanceById(replyId);
				}
				String replyContent="";
	...
	private List<MommyPostInfo> deleteUnchosePostInfoInstance(List<MommyPostInfo> postInfoList,long doctorRingLableId){
		List<MommyPostInfo> returnPostInfoList=null;
		if(postInfoList!=null){
			returnPostInfoList=new ArrayList<MommyPostInfo>();
			for(int postInfoSize=0;postInfoSize<postInfoList.size();postInfoSize++){
				MommyPostInfo postInfoIntance=postInfoList.get(postInfoSize);
				if(postInfoIntance!=null){
					long postInfoId=postInfoIntance.getId();
					MommyPostInfoLabel postInfoLabelInstance=mommyPostInfoLabelBo.checkSomePostInfoLabel(postInfoId, doctorRingLableId);
	 ...
	 * @return 处理完成的医生圈标签
	 */
	private List<Object[]> ringLabelHandle(List<MommyRingLabel> doctorRingLabelList){
		...
		if(doctorRingLabelList!=null){
			...
			for(int labelListSize=0;labelListSize<doctorRingLabelList.size();labelListSize++){
				...
				MommyRingLabel ringLabelInstance=doctorRingLabelList.get(labelListSize);
