			...
			success:function(result){
				if(result.mes=="无引导"){
					...
					for(var i=0;i<result.guidePictureList.length;i++){
						//ale("第"+(i+1)+"个引导页的名称是："+result.guidePictureList[i].pageName);
...
//增加获取用户是否已经看过图片的cookie
function getCookieManage(){
	$.ajax({
		type:'post',
		url:host+'getGuideInfo.action',
		cache:false,
	    async:false, 
		data:{action:"addGuideCookie"},
		success:function(result){
			if(result.mes=="操作成功"){
				window.location.href="main.html";
			}else{
