...
$(function () {
            ...
            for (var ii = 0; ii < addressArr.length; ii++) {
                for (var mm = 0; mm < addressArr[ii].length; mm++) {
                    if (mm == 0) {
                        ...
                        var isDel=doctorClinicTimeSegmentList[0].isDel;
    ...
    for (var kk = 0; kk < 14; kk++) {
        if ($('#recentTime>li').eq(kk).find('.weekdate').text() == useChooseDate) {
            ...
            $('#recentTime').scrollLeft((kk-3)*(0.13)*$('#recentTime').width());
        }
    }
    ...
    for (var qq = 0; qq < checkedTime.length; qq++) {
        if ($(checkedTime).eq(qq).attr('index') == $('.activeAppoint').attr('index')) {
            if ($(checkedTime).eq(qq).find('img').attr('src') ==hostMain+ 'images/images_family_doctor/true.png') {
                var timeSet=$(checkedTime).eq(qq).attr('data-time-list');
                var isCanUse=$(checkedTime).eq(qq).attr('data-use-list');
                var isDel=$(checkedTime).eq(qq).attr('data-isDel-list');
                if(timeSet.trim()==''){
                    ...
                    if(!s){
                        $('#chooseAppointmentTime').append('<li style="background:yellow" data-clinicEndTime="'+$(checkedTime).eq(qq).attr('data-clinicEndTime')+'">加号' + $(checkedTime).eq(qq).attr('data-isAddClinic') + '</li>');
                        $('h5').html('<input type="button" style="background: #ccc"/> 表示该时间已被预约&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" style="background: yellow"/> 表示门诊可加号数');
                    }else{
...
//                    }
    ...
    $('.address').click(function () {
        ...
        var isDel1 = $(this).attr('data-isDel-list');
        if(timeSet1.trim()==''){
            $('#chooseAppointmentTime').append('<li data-clinicEndTime="'+$(this).attr('data-clinicEndTime')+'">加号' + $(this).attr('data-isAddClinic') + '</li>');
        }else{
            var dateTimeList1 = timeSet1.split(',');
            var dateUseList1 = isCanUse1.split(',');
            var isDel1 = isDel1.split(',');
            var dateTimeHtml1 = '';
            var s;
            ...
            if(!s){
                $('#chooseAppointmentTime').append('<li style="background:yellow" data-clinicEndTime="'+$(this).attr('data-clinicEndTime')+'">加号' + $(this).attr('data-isAddClinic') + '</li>');
                $('h5').html('<input type="button" style="background: #ccc"/> 表示该时间已被预约&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" style="background: yellow"/> 表示门诊可加号数');
            }else{
                $('h5').html('<input type="button" style="background: #ccc"/> 表示该时间已被预约&nbsp;&nbsp;&nbsp;&nbsp;');
            }
            ...
            success: function (result) {
                if(result.mes=='成功'){
                    ...
                    if(isSession=='yes'){
                        ...
                        window.location.href = 'payAppointment.html?' + result.orderInfoClinic.id;
                    }
                }else{
                    ale(result.mes);
                }
