...
var hospitalArr=[];
$(function () {
    organSetMeatOrderId=decodeURIComponent(window.location.search.substring(1));
    getFdServiceOrder();
...
function getFdServiceOrder(){//获取订单信息
    $.ajax({
        type: 'post',
        async: false,
        cache: true,
        ...
        success: function (result) {
            if (result.mes == '请登录') {
                ...
                var organSetMeatOrder=result.organSetMeatOrder;
                $('#service_time').html('<p class="padding"><span class="left">'+organSetMeatOrder.organSetMeal.packageName+'</span><span class="right"></span></p> ' +
                '<p class="gray_s"></p> ' +
                '<p class="padding"><span class="left">服务对象</span><span class="right">'+result.organSetMeatOrder.babyInfo.babyName+'</span></p> ' +
                '<p class="gray_s"></p> ' +
                '<p class="padding"><span class="left">服务到期时间</span><span class="right">'+organSetMeatOrder.serviceEndTime+'</span></p>');
                $('#accountBalance').html(parseFloat(organSetMeatOrder.userInfo.accountBalance,2).toFixed(2));
                if(organSetMeatOrder.organSetPro.isOfflinePay!="Y"){
                    $('#offline_payment').parent().css({display:'none'});
                    $('#offline_payment').parent().prev().css({display:'none'});
                }
                if(result.organSetMeatOrder.orderStatus=="已付款"){
                    $('footer').hide();
                }
                $('#money').html('&yen;'+parseFloat(organSetMeatOrder.totalPrice,2).toFixed(2));
                $('#pay_subtotal>span').html(parseFloat(organSetMeatOrder.totalPrice,2).toFixed(2));
                balanceAll=parseFloat(organSetMeatOrder.userInfo.accountBalance,2);
                totalPrice=parseFloat(organSetMeatOrder.totalPrice,2);
                orderNum=parseFloat(organSetMeatOrder.orderNum);
                //hospitalArr=[organSetMeatOrder.fdServicePackage.hospitalBasicInfo.hospitalLname,organSetMeatOrder.fdServicePackage.hospitalBasicInfo.address]
            }
        },
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.mes == '请登录') {
                ...
                if(method=='线下支付'){
                    window.location.href="package_offline_success.html";
                }else{
                    window.location.href="package_online_success.html";
                }
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
