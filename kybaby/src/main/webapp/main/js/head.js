 ...
 */
//var keyWord1=sessionStorage.getItem('key');
//sessionStorage.removeItem('key');
//sessionStorage.setItem('keyWord1',keyWord1);
...
//    } );
    ...
    $('.block:last').find('.kd').css({opacity:0,filter:'alpha(opacity=0)'});
    ...
    for(var i=0;i<arrBasicUrlSearch.length;i++){
    	if(arrBasicUrlSearch[i].split("=")[0]=="value"){
    		curHeadValue=arrBasicUrlSearch[i].split("=")[1];
    		...
    		monthOld=arrBasicUrlSearch[i].split("=")[1];
        		...
        		if(proData == undefined || proData == null || proData == 'undefined'){
	        		...
	        		nowData=(((Math.round((curHeadValue)*100)/100-15.0)*159.96));
    ...
    //$("#num").text(curHeadValue);
});
function get(){
    var a=(Math.round(($(".box").scrollLeft()/159.96+15.0 )*10.0)/10.0).toString();
    if(a%1==0){
        a+=".0";
    }
    $("#num").text(a);
