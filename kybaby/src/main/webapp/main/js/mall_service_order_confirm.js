    ...
    $.ajax({
        ...
        success: function (result) {
            if(result.mes=='请登录'){
                ...
                '<p class="product_specifications">'+propVals+'</p>' +
                '<p class="product_price" style="display: none">￥<span>'+parseFloat(url_id.split('&')[1]).toFixed(2)+'</span></p>' +
                '</div>');
        ...
        success: function (result) {
            if(result.mes=='请登录'){
                ...
                if(result.b2cGoodsOrder!=null){
                    ...
                    '<p class="product_specifications">'+propVals+'</p><p class="product_price" style="display: none">￥<span>'+parseFloat(orderdetail.price).toFixed(2)+'</span></p></div>');
