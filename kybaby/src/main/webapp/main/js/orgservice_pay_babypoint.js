...
function modify(){
    window.location.href='appointmentPatient.html?'+orderInfoId;
}
$(function(){
    ...
    if(answer==true){
        $.ajax({
            ...
            data:{
                ...
                "userChildcareAppointmentInfo.id":orderInfoId,
                "userChildcareAppointmentInfo.status":"取消"
            },
            success:function(result){
                if (result.mes == '请登录') {
                    ale('请登录', '24px');
        ...
        function(res){
            if(res.err_msg == "get_brand_wcpay_request:ok" || res.err_msg == "get_brand_wcpay_request：ok") {
                ...
                $.ajax({
                    ...
                    success:function(result){
                        window.location.href="orgservice_babymessage.html?"+orderInfoId;
                        if(result.mes=="操作成功"){
                            //ale("订单更新成功");
                        }
                    },
