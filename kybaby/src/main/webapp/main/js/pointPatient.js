...
$(function () {
    ...
    if(orgId == 'undefined' || orgId == undefined||orgId==''){
        orgId='';
        $('#erke,.point').show();
        $('.erke').addClass('active');
    }
    else{
        if(orgId=='图文咨询'){
            orgId='';
            $('#graphic_consulting,.buy').show();
            $('.tuwen').addClass('active');
            $('#header>.header-left').attr('onclick','window.location.href="consult_doctorlist.html"');
        }else{
            $('#erke,.point').show();
            $('.erke').addClass('active');
            orgId=decodeURIComponent(window.location.search.substring(1).split('&')[1]);
            $('#header>.header-left').attr('onclick','window.location.href="orgservice_doctorlist.html?'+orgId+'"');
        }
    }
...
$(function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            "doctorInfo.id":serDocId//得到单个医生信息传此参数，医生列表不需要传参
        },
        success: function (result) {
            if (result.mes == '请登录') {
                ale('请登录', '24px');
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#recentTime').scrollLeft((recentArr[0]-3)*(0.13)*$('#recentTime').width());
            ...
            if (list.length == 0) {
                $('#footer p.point').html('暂无服务').addClass('dark_button').removeAttr('onclick');
                $('#no_server').show();
                $('#pay_server').hide();
            }
        },
    ...
    $('#tab>div').click(function () {
        ...
        if($(this).hasClass('erke')){
            $('#erke,p.point').show();
            $('#graphic_consulting,p.buy').hide();
        }else{
            $('#erke,p.point').hide();
            $('#graphic_consulting,p.buy').show();
        }
    });
});
//医生简介多余字数处理
$(function () {
    ...
    $.ajax({
        ...
        cache:false,
        async:true,
        data:{action:"getConsultDoctorInfo","doctorInfo.id":serDocId},
        success:function(result){
            if (result.mes == '请登录') {
                ale('请登录', '24px');
                window.location.href = "login.html";
                ...
                var consultDoctorInfo=result.consultDoctorInfo;
                if(consultDoctorInfo==null){
                    $("#no_server").show();
                    $("#pay_server").hide();
                    $('#footer p.buy').html('暂无服务').addClass('dark_button').removeAttr('onclick');
                }else{
                    $("#no_server").hide();
                    $("#pay_server").show();
                    $("#pay_server>p").text('￥'+parseFloat(consultDoctorInfo.consultMoney).toFixed(2)+'/次');
                }
            }
        },
        error: function(XMLHttpRequest, textStatus, errorThrown) {
    ...
    $.ajax({
        ...
        success:function(result){
            if (result.mes == '请登录') {
                ...
                window.location.href='consult_pay_service.html?'+serDocId;
            } else if (result.mes == "已购买所选医生服务") {
                ale('您已购买过该医生咨询服务，请到个人中心-我的咨询发起咨询');
            }
