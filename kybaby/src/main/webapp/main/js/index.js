...
function getMessageList(){
    $.ajax({
        ...
        success:function(result){
            animal(result.userInfo.birthday);
            if(result.mes == "成功"){
...
function animal(date){//生肖的操作
    var zodiac=date,type='roll', len=14,img=$('#message>img');
    if(date.indexOf('-')>-1){
        zodiac=parseInt(zodiac.substr(0,4));
        zodiac=(zodiac-1900)%12;//1900'鼠'
        //zodiac=4;
        switch (zodiac){
            case 0://鼠
                img.prop('src','images/images_main/zodiac_mouse.gif');
                type='jump';
                break;
            case 1://牛
                img.prop('src','images/images_main/zodiac_cow.gif');
                type='jump';
                break;
            case 2://虎
                img.prop('src','images/images_main/zodiac_tiger.gif');
                type='jump';
                break;
            case 3://兔
                img.prop('src','images/images_main/zodiac_rabbit.gif');
                type='jump';
                break;
            case 4://龙
                img.prop('src','images/images_main/zodiac_dragon.gif');
                type='jump';
                break;
            case 5://蛇
                img.prop('src','images/images_main/zodiac_snake.gif');
                type='jump';
                break;
            case 6://马
                img.prop('src','images/images_main/zodiac_horse.gif');
                type='jump';
                break;
            case 7://羊
                img.prop('src','images/images_main/zodiac_sheep.gif');
                type='jump';
                break;
            case 8://猴
                img.prop('src','images/images_main/zodiac_monkey.gif');
                type='jump';
                break;
            case 9://鸡
                img.prop('src','images/images_main/zodiac_chicken.gif');
                type='jump';
                break;
            case 10://狗
                img.prop('src','images/images_main/zodiac_dog.gif');
                type='jump';
                break;
            case 11://猪
                img.prop('src','images/images_main/zodiac_pig.gif');
                type='jump';
                break;
        }
    }
    if(type=='jump'){//人的生肖跳
        for(var i= 0;i<len;i++){
            var percentage=(100-(i+1)*(100/len))+"%";
            if(i%2==0){
                $('#message').animate({left:percentage,bottom:'50px',height:'88px'},4000/len);
            }else{
                $('#message').animate({left:percentage,bottom:'50px',height:'68px'},4000/len);
            }
        }
    }else if(type=='roll'){//人的生肖滚
        $('#message>img').addClass('roll');
        for(var i= 0;i<len;i++){
            var percentage=(100-(i+1)*(100/len))+"%";
            $('#message').animate({left:percentage},4000/len);
        }
    }
    setTimeout(function () {
        $('#message').css({'border':'none','width':'98px'}).attr('onclick','window.location.href="messageCenter.html"');
        if(type=='jump'){
            //$('#message>img').addClass('scale');
            $('#message>img').removeClass('roll');
        } else if(type=='roll'){
            $('#message>img').removeClass('roll');
        }
        $('#message>div').css({'background':'rgba(255,0,0,1)'});
    },4000);
}
//获取手机当前位置
