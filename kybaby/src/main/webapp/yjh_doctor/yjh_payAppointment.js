...
$(function(){
    ...
    $.ajax({
        ...
        data:{action:"getClinicOrderInfo",
            "orderInfoClinic.id":orderInfoId,"userInfo.id":userId
        },
...
$('.payWay tr').click(function(){
    ...
    $('.icon').attr('src',hostMain+'images/images_family_doctor/select.png');
    $(this).addClass('choosePayWay');
    $(this).find('.icon').attr('src',hostMain+'images/images_family_doctor/true.png');
});
//是否选择福利
$('.preferential').click(function () {
    ...
    if($('#discountMoney').prop('class')=='preferential_payment'){
        ...
        $(this).find('img').prop('src',hostMain+'images/images_family_doctor/select.png');
    }
    ...
    if(answer==true){
        $.ajax({
            ...
            data:{
                ...
                "orderInfoClinic.orderStatus":"用户取消","userInfo.id":userId
            },
            success:function(result){
                if (result.mes == '请登录') {
                    ale('请登录', '24px');
                    ...
                    var isSession=sessionStorage.getItem('session');
                    if(isSession=='yes'){
                        window.location.href = 'yjh_appointmentPatient.html?' + result.orderInfoClinic.id;
                    }else{
...
function payProduct() {
    ...
    $.ajax({
        ...
        data: {
            ...
            "orderInfoClinic.isPlus": isPlus,"userInfo.id":userId
        },
        success: function (result) {
            if (result.mes == '请登录') {
                ale('请登录', '24px');
...
function onDeviceReady(){
    ...
    $.ajax({
        ...
        data: {
            action: "getAppToken",
            'comments':userInfoArr[1],"userInfo.id":userId
        },
        success: function (result) {
            var str=decodeURIComponent(result.extraList);
            ...
            if(obj.return_code=='SUCCESS'){
                YJH. H5ModuleManager.requestPayment(function(res){
                    ...
                    0,
                    {'userid':userInfoArr[0],'cid':obj.data[0].id,'subject':obj.data[0].name,'totalFee':payAmount,'token':result.userToken ,'body':obj.data[0].content}
                );
            }else{
                alert('支付失败');
            }
...
function paySuccess(payAmount){
    $.ajax({
        ...
        data: {
            ...
            "orderInfoClinic.discountMoney": useDiscountMoney,"userInfo.id":userId
        },
        success: function (result) {
        }
    });
    ...
    $.ajax({
        type: 'post',
        url: hostMain + 'sendMessageByClinic.action',
        cache: false,
        data: {action: "toUser", "orderInfoClinic.id": orderInfoId, sendWords: "预约门诊","userInfo.id":userId},
        success: function (result) {
        }
    });
    $.ajax({
        ...
        url: hostMain + 'sendMessageByClinic.action',
        cache: false,
        data: {action: "toDoctor", "orderInfoClinic.id": orderInfoId, sendWords: "预约门诊","userInfo.id":userId},
        success: function (result) {
        }
    });
    ale('支付成功');
    //ale("跳转到支付成功页面");
    window.location.href = "yjh_myAppointment.html";
}