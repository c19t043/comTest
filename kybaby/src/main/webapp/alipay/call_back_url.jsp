...
%>
 ...
 <script>
    if(<%=verify_result %>) {
		//支付成功的处理代码
		//订单编号
		var orderNum=<%=out_trade_no%>;
		//alert("orderNum=="+orderNum);
		//test
		//alert("total_fee=="+
		
		var orderAmount=<%=session.getAttribute("fee") %>;
		orderAmount=Math.round(orderAmount*100);
		window.location.href="../main/paysuccess.html?"+orderAmount;
		//alert("跳转到支付成功页面");
    } else {
    	//支付失败的处理代码
    	//alert(<%=out_trade_no%>);
    	window.location.href="main/payfail.html";
    }
 </script>
  </body>
</html>