...
var organSetMeaId=decodeURIComponent(window.location.search.substring(1).split('&')[0]);
$(function () {
    getLocationFirst();
});
function orgList(){
    $.ajax({
        ...
        cache:false,
        async:true,
        data:{action:"getSetMealHospitalList",
            "organSetMeal.id":organSetMeaId,
            ...
            if(result.mes == '请登录'){
                ...
                if(result.hospitalBasicInfoList!=null){
                    ...
                    for (var i = 0; i < length; i++) {
                        ...
                        var doctors='';
                        var isOpen='Y';
                        if(hospitalBasicInfoList.businessNameList==null){
                            doctors='<span class="number">敬请期待</span>';
                            isOpen='N';
                        }else{
                            doctors='<span class="number">套餐服务</span>';
                        }
                        if(hospitalBasicInfoList.hospitalType=='社区医院'){
                            $("#community_list").append('<div class="community_div" onclick="goLink('+hospitalBasicInfoList.id+',\''+isOpen+'\')"><div class="community_header"><img src="'+showImgPath+'"/></div>' +
                            ...
                            $("#hospital_list").append('<div class="community_div" onclick="goLink('+hospitalBasicInfoList.id+',\''+isOpen+'\')"><div class="community_header"><img src="'+showImgPath+'"/></div>' +
                                '<div class="community_information"><p class="community_name">'+hospitalBasicInfoList.hospitalLname+'</p>' +
                                '<p>'+doctors+'</p><p>'+hospitalBasicInfoList.address+'</p></div>' +
                                '<div class="community_distance">'+ parseFloat(hospitalBasicInfoList.toUserDistance).toFixed(2) + 'km</div></div>'
                            );
...
//获
function goLink(id,isOpen){
    if(isOpen=="Y"){
        window.location.href='package_confirm.html?'+id+'&'+organSetMeaId;
    }else{
        ale('该机构暂未开通服务');
    }
