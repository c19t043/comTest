        ...
        success:function(result){
            if(result.mes=="操作成功"){
                ...
                for(var i=0;i<result.allRingCategory.length;i++){
                    ...
                    $('#circle_type').append(
                        "<li data_state='"+result.allRingCategory[i][2]+"' onclick=\"getSomeCategory('"+result.allRingCategory[i][0]+"',this)\" class='"+cla+"' zjid='"+result.allRingCategory[i][0]+"'>"+result.allRingCategory[i][1]+"</li>"
                    );
                }
...
//点击某一个妈妈圈，进入到妈妈圈内获取到妈妈圈内帖子的详细列表
function getSomeDoctorRingInfo(zjid,typename){
    ...
    window.location.href = "chatAll.html?"+zjid+"&&"+encodeURIComponent(typename)+"&&"+returnMark+"&&全部";
}
$(function(){
    getAllRingCategory();
});
