...
//                    宝宝信息
                ...
                $("#result_ul").html('<li class="flex_cont"><p>姓名：</p><p class="flex_item">'+userInfo.asqUserName+'</p></li>' +
                            ...
                            for (var j = 0; j < taotiList.asqQuestionsSet.length; j++) {
                                ...
                                if (asqQuestions.asqQuestionRecordList != null) {
                                    ...
                                    for (var l = 0; l < asqQuestions.asqQuestionRecordList.length; l++) {
                                        score += asqQuestions.asqQuestionRecordList[l].score;
                                        //信息汇总各问题得分情况循环得分情况
                                    ...
                                    $("#list_two li").eq(i).append('<span>'+score+'</span>');
                                }
                            }
                        }
                    }
...
//                    信息汇总
                if(result.asqResultScoreEx.status=="待审阅"){
                    ...
                    if(result.asqResultScoreExList!=null){
                        for(var i = 0;i<result.asqResultScoreExList.length;i++){
                            ...
                            $("#list_one").append('<li><span>'+asqResult.asqTaoti.titalName+'</span><span>'+asqResult.sumScore+'</span><span>'+asqResult.resultDescription+'</span></li>');
                        }
                    }else{
                ...
                //综合信息汇总
                for(var n = 0;n<$("#question_information .project_title").length;n++){
                    if($("#question_information .project_title").eq(n).text()=="综合问题"){
                        if($(".project_title").eq(n).next(".problem_item").html()!=null){
                            $("#comprehensive>.problem_record").html($(".project_title").eq(n).next(".problem_item").html());
                            $("#comprehensive_problem").show();
                        }
                    }
                }
...
//                   亲子活动
                if(result.asqParentChildActivityList!=null){
                    for(var i = 0;i<result.asqParentChildActivityList.length;i++){
                        ...
                        $("#active_information").append('<div class="active_div">'+activityList.activityContent+'</div>');
                    }
                }else {
