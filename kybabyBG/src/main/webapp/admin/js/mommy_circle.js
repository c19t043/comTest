...
function getAllRingCategory(){
    $.ajax({
        type:'post',
        url:ringHost+'getMommyRringInfo.action',
        cache:false,
        async:true,
...
function showFirstLevel(){
    $.ajax({
        ...
        success:function(result){
            if(result.mes != "操作成功" || result.mommyTheTypeInfoList == null){
                return false;
            }
            $('#circle_type').empty();
            $.each(result.mommyTheTypeInfoList,function(i,content){
                $('#circle_type').append(
                    "<tr >"+
                    "<td class='a'>"+
...
function getSecondCircleList(searchId){
    $.ajax({
        ...
        type:"POST",
        data:{
            action:"allMommyRingType",
            "theTypeInfo.id":searchId
        },
        async:"false",
        success:function(result){
            $("#circle_lidec").empty();
            if(result.mes!="操作成功" || result.mommyRingTypeList == null){
                return false;
            }
            $.each(result.mommyRingTypeList,function(i,content){
                var mark = '';
...
function secondAddOrChange(ty,obj){
    if(ty=="change"){
        ...
        var Data = {
            "action":"saveOrUpdateMommyRingType",//action
            "mommyRingType.id":$(obj).data("id"),//id
            "mommyRingType.subclassName":$(divObj).find(".secondName").val().trim(),//名字
            "mommyRingType.remark":$(divObj).find(".secondRemark").val().trim(),//说明
            "mommyRingType.isStart":$(divObj).find(".secondStart").val(),//是否启用
            "mommyRingType.isRecommend":$(divObj).find(".secondSub").val(),//是否推荐

            "mommyRingType.typeImg":$(divObj).find(".loadimg_base64").val().trim(),//图片base64
            "mommyRingType.typeId":$(obj).data("typeid")//一级ID
        };
    }else if(ty=="add"){
        var Data = {
            "action":"saveOrUpdateMommyRingType",//action
            "mommyRingType.subclassName":$("#addSecondName").val().trim(),//名字
            "mommyRingType.remark":$("#addSecondDesc").val().trim(),//说明
            "mommyRingType.isStart":$("#addSecondQY").val(),//是否启用
            "mommyRingType.isRecommend":$("#addSecondTJ").val(),//是否推荐

            "mommyRingType.typeImg":$("#addSecondImg").val().trim(),//图片base64
            "mommyRingType.typeId":$(":selected",$("#ssfl")).attr("zjid")//一级ID
        };
    }
    var mark = true;
