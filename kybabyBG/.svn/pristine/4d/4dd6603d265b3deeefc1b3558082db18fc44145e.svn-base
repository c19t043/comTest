...
function addNewPostInfo(){
	var postInfoTitle=$.trim($("#note_title").val());
	var postInfoContent=$.trim($("#noteContent").html());
	...
	var postInfoStatus=$.trim($("#state").val());
...
function read_info(obj){
	...
	getSomePostInfoLabelIdList($(obj).attr('data_id'));
...
function changePostInfoInstance(){
	...
	var postInfoId = $("#note_title").attr('zjid');
	var postInfoTitle=$.trim($("#note_title").val());
	var postInfoContent=$.trim($("#noteContent").html());
	var ringTypeId=$('#secondLev').find('option').filter(function(){
			return $(this).get(0).selected == true;
...
//获取帖子的标签
function getSomePostInfoLabelIdList(postInfoLabelId){
    ...
    async:false, 
		...
		success:function(result){
			if(result.mes != '操作成功' || result.somePostInfoLabelIdList == null){
				return false;
			}
			$.each(result.somePostInfoLabelIdList,function(i,content){
