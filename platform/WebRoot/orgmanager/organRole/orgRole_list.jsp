...
<html>
	...
	<s:actionmessage theme="popwind"/>
				...
				<td height="43" align="center" class="text3">
					<s:form action="/orgOperatorManager/orgRoleManager.action" name="query_list_form" id="query_list_form" theme="simple">
						<table width="98%"  border="0" cellpadding="0" cellspacing="0" class="text3">
							<tr>
								<td width="8%" height="41" align="right">角色名称：</td>
								<td width="12%" align="left">
									<input name="organRole.roleName"  type="text" class="input1" value="${organRole.roleName}"/>		
								</td>
								<td width="8%" height="41" align="right">角色编码：</td>
								<td width="12%" align="left">
									<input name="organRole.roleCode"  type="text" class="input1" value="${organRole.roleCode}"/>		
								</td>
								<td width="19%" align="left">
									<input name="action"  type="hidden" class="input1" value="queryOrganRoleByPage"/>
					...
					<div class="eXtremeTable">
						<s:if test="#request.list.size>0">
							<com.java.ec:table items="list" var="p" tableId="list" border="0"
								action="${pageContext.request.contextPath}/orgOperatorManager/orgRoleManager.action?action=queryOrganRoleByPage"
								...
								<com.java.ec:row >
									...
									<com.java.ec:column property="null" title="操作" width="10%">
										<a href="javascript:void();" onclick="distributeUser('${p.id}')">分配用户</a>
										<a href="javascript:void();" onclick="distributeModule('${p.id}')">分配模块</a>
									</com.java.ec:column>
								</com.java.ec:row>
							</com.java.ec:table>
						</s:if>
		...
		<script type="text/javascript">
			$j = jQuery.noConflict();
			//分配模块
			function distributeModule(id){
		 		var page2page = "ID="+id;  //把这个page2page传到另外一个页面
		 		environment.dialog.open({
					url : '${pageContext.request.contextPath}/orgmanager/organRole/orgModule_select_main.jsp?' + page2page, 
			...
			//分配用户
			function distributeUser(id){
		 		var page2page = "ID="+id;  //把这个page2page传到另外一个页面
		 		environment.dialog.open({
					url : '${pageContext.request.contextPath}/orgmanager/organRole/orgOperator_select_main.jsp?' + page2page, 
					width : window.getCoordinates().width-300,
				    height : window.getCoordinates().height-50,
					icon : '${pageContext.request.contextPath}/product/theme/winxp/resource/skin/blue/images/display.gif',
					title : '分配用户'
			...
			function doAdd(action) {
				action="${pageContext.request.contextPath}/orgOperatorManager/orgRoleManager.action?action=toAddOrUpdatePage";
			...
			function doEdit(action) {
				...
				window.location.href=action + "&organRole.id="+$checked.val();
			}
		</script>
  	</body>
</html>
