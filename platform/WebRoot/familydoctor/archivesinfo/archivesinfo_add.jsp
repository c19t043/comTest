...
<html>
	<head>
  		...
  		<script type="text/javascript">
	  		...
	  		//选择用户类型
	  			$("userTypeName").addEvent("click",function() {
			 		var page2page = "text=userTypeName";  //把这个page2page传到另外一个页面
			 		environment.dialog.open({
			...
			//返回
			function doBack(){
				window.location.href = "${pageContext.request.contextPath}/familydoctor/archivesInfo/getArchivesInfoList.action";
			}
  	...
  	<body>
		...
		<form id="form_handle" name="form_handle" action="<s:url namespace="/familydoctor/archivesInfo" action="saveOrUpdateArchivesInfo" includeParams="true"/>" method="post" >
			<input name="save" id="save" type="submit" value="save" style="display: none;">
			<input type="hidden" name="archivesInfo.id" value="${archivesInfo.id}">
			<input type="hidden" name="archivesInfo.userInfo.id" value="${archivesInfo.userInfo.id}">
			<div class="eXtremeTable">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<table width="100%" class="tableRegion2">
						<tr>
							<td class="inputLabel">预留手机号：</td>
							<td width="30%">
								<input type="text" name="archivesInfo.archivesMobile" value="${archivesInfo.archivesMobile}" class="validate['required','length[20]'] text3">
								<span class="text4">*</span>
							</td>
							
							<td class="inputLabel">接种证编号：</td>
							<td width="30%" >
								<input type="text" name="archivesInfo.inoculationCode" value="${archivesInfo.inoculationCode}" class="validate['length[20]'] text3" width="70%">
							</td>
					  	</tr>
					  	<tr>
							<td class="inputLabel">儿童姓名：</td>
							<td width="30%" >
								<input type="text" name="archivesInfo.childrenName" value="${archivesInfo.childrenName}" class="validate['length[20]'] text3" width="70%">
							</td>
							
							<td class="inputLabel">儿童出生日期：</td>
					            <td>
					                <input type="text" name="archivesInfo.childrenBirthday" value="${archivesInfo.childrenBirthday}"
					  	...
					  	<tr>
							...
							<td width="30%" >
								<input type="text" id="hospitalName" value="${archivesInfo.hospitalBasicInfo.hospitalLname}" class="validate['required','length[20]'] text3" width="70%">
								<input type="hidden" id="hospitalId" name="archivesInfo.hospitalBasicInfo.id" value="${archivesInfo.hospitalBasicInfo.id}" >
							</td>
							<td class="inputLabel">用户类型：</td>
							<td width="30%" >
								<input type="text" id="userTypeName"  name="archivesInfo.userType" value="${archivesInfo.userType}" class="validate['required','length[20]'] text3" width="70%">
							</td>
					  	</tr>
					  	
					  	<tr>
							<td class="inputLabel">是否关联 Y：是 N：否</td>
							<td width="30%">
								<s:select cssStyle="width:100px;" cssClass="text3" list="#{'Y':'Y','N':'N'}" listKey="key" listValue="value" 
									theme="simple" name="archivesInfo.isRelation" value="archivesInfo.isRelation"/>
								<span class="text4">*</span>
							</td> 	
							<td class="inputLabel">有效时长：</td>
							<td width="30%" >
								<input type="text" name="archivesInfo.effectiveTime" value="${archivesInfo.effectiveTime}" class="validate['length[20]'] text3" width="70%">
							</td>
					  	</tr>
					  	<tr>
							<td class="inputLabel">最大月龄限制（到期月龄）:</td>
							<td width="30%">
								<input type="text" name="archivesInfo.maxMonthAge" value="${archivesInfo.maxMonthAge }" class="text3" width="70%">
							</td> 	
							<td class="inputLabel">用户来源：</td>
							<td width="30%" >
								<input type="text" name="archivesInfo.userFrom" value="${archivesInfo.userFrom}" class="text3" width="70%">
							</td>
					  	</tr>
					</table>
				<br>
