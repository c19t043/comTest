	...
	<head>
		...
		<!--<script type="text/javascript" src="${pageContext.request.contextPath}/platform/theme/public_js/utils2.js"></script>-->
  		...
  		<script type="text/javascript">
			...
			function editId(){
				<%String val = request.getParameter("kyUserInfoId");%>
				console.log(<%=val%>);
				var userId = <%=val%>;
				if(userId) $j(":input[name='phPeopleBasicInfo.familyAccountInfo.id']").val(userId);
				//console.log($j(":input[name='phPeopleBasicInfo.id']").val());
			}
    ...
    //家族史和残疾（ disability）
  	...
  	<body>
		...
		<form id="form_handle" name="form_handle" action="<s:url namespace='residentsFile' action='saveOrUpdatePhPeopleBasicInfo' includeParams='true'/>" method="post" >
			...
			<input type="hidden" name="phPeopleBasicInfo.familyAccountInfo.id" value="${phPeopleBasicInfo.familyAccountInfo.id}">
			<input type="hidden" name="phPeopleBasicInfo.consultBabyInfo.id" value="${phPeopleBasicInfo.consultBabyInfo.id}">
					...
					<table width="100%" class="tableRegion2">
						<tr>
                            ...
                            <td width="23%">
                                ...
                                class="validate['required','length[20]'] text3 Wdate" onFocus="WdatePicker({isShowClear:true,dateFmt:'yyyy-MM-dd',readOnly:true});">
                                <span class="text4">*</span>
                            </td>
					  	</tr>
                        <tr>
