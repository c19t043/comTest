	...
	<body id="tree_main">
		...
		<script type="text/javascript">
			...
			$j(document).ready(function(){
				var tmp_name = $j("#"+tname,opener.document).val();
				var nameArr = (tmp_name+",").split(",");
				var IDArr = ($j("#"+tID,opener.document).val()+",").split(",");
				$j("#checkedObject").html(tmp_name);
				for(var i=0,len=nameArr.length;i<len;i++){
					if(nameArr[i])
						objectList.push(new obj(IDArr[i],nameArr[i]));
				}
				$j("#addObjectButton").bind("click",select);
				$j("#objectIframe").attr("src","${pageContext.request.contextPath}/AsqTest/toAsqTaotiSelectPage.action?asqTaoti.flag=<%=flag%>");
			});
			...
			function select(){
				...
				if(id.length>0){
					...
					$j("#"+tID,opener.document).val(id);
					$j("#"+tname,opener.document).val(name);
				}
