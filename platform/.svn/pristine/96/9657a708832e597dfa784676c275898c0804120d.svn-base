...
<script type="text/javascript">
	...
	window.addEvent('domready', function(){
		...
		var serviceItemIdsName = environment.dialog.getOpener().$("parentName").value;
		var serviceItemIds = environment.dialog.getOpener().$("parentID").value;
			
		if(serviceItemIds!=null&&serviceItemIds.length>0){
			$j('#cheakObject').html(serviceItemIdsName);
			var ids = serviceItemIds.split(",");
			var names = serviceItemIdsName.split(",");
			for(var i=0;i<ids.length;i++){
				if(ids[i]!=null&&ids[i].length>0){
					insertObject(ids[i],names[i])
				}
			}
		}
		...
		$('objectIframe').src = "${pageContext.request.contextPath}/medicalorgandbusiness/setpro/getParentPro.action";
	});
	...
	function select(){
		...
		for(var i=0,len=set_pro_Object_list.length;i<len;i++){
			id+=set_pro_Object_list[i].id+",";
			name+=set_pro_Object_list[i].name+",";
		}
		...
		environment.dialog.getOpener().$("parentID").value = id;
 		environment.dialog.getOpener().$("parentName").value = name;
 		
 		$j(environment.dialog.getOpener().$("parentID")).click();
	...
	function insertObject(id,name){
		...
		var addFlag = true;
		for(var i=0,len=set_pro_Object_list.length;i<len;i++){
			if(set_pro_Object_list[i].id == id){
				set_pro_Object_list.splice(i, 1);addFlag=false;break;
			}
		}
		
		if(addFlag) {
			set_pro_Object_list = new Array();
			set_pro_Object_list.push(set_pro_Object);
		}
		
		var content = "";
		for(var i=0,len=set_pro_Object_list.length;i<len;i++){
			content+="{"+set_pro_Object_list[i].name+"}";
		}
		$j('#cheakObject').html(content)
	}
</script>