...
// 				window.location.href=action;
			...
			function doAdd(action) {
				...
				window.location.href=action+"?phPeopleBasicInfo.infoOwner=mum";
			}
			// 第一次产检
			function doPreFirst(action) {
			...
			function doPostpartum6(action){
				 var items = EcTable.getCheckedItems();
	                ...
	                window.location.href=action + "?phPeopleBasicInfo.id="+id+"&phPostpartumFollowRecord.followUpOpportunity=0";
			}
			//产后40天
			function doPostpartum40(action){
				 var items = EcTable.getCheckedItems();
	                if(items.length==0){
	                    showMsg("至少选择一项！");
	                    return;
	                ...
	                window.location.href=action + "?phPeopleBasicInfo.id="+id+"&phPostpartumFollowRecord.followUpOpportunity=1";
			}
			// 编辑信息 
		...
		<table width="100%" height="45"  border="0" cellpadding="1" cellspacing="1" bgcolor="#B7C6DD">
			<tr align="center" valign="middle" bgcolor="#F6F9FE">
				<td height="43" align="center" class="text3">
					<s:form action="/residentsFile/phMumPeopleBasicInfoList.action" name="query_list_form" id="query_list_form" theme="simple">
					...
					<div class="eXtremeTable">
						<s:if test="#request.list.size>0">
							<com.java.ec:table items="list" var="p" tableId="list" border="0"
								action="${pageContext.request.contextPath}/residentsFile/phMumPeopleBasicInfoList.action?accountId=${p.familyAccountInfo.id }"
								...
								<com.java.ec:row>
									...
									<com.java.ec:column property="name" title="姓名" width="10%"/>
									<com.java.ec:column property="idCardNum" title="身份证号" width="10%"/>
									<com.java.ec:column property="contactPhone" title="联系人电话" width="10%"/>
