		...
		if(ids){
			...
			for(var i=0,len=idArr.length;i<len;i++){
				if(nameArr[i].length>0)
					objectList.push({"name":nameArr[i],"id":idArr[i]});
			}
			$j('#cheakObject').html(names);
		}
		$('addObjectButton').addEvent("click", select);
		
		$('objectIframe').src = "${pageContext.request.contextPath}/operationmanage/getHospitalInfoList.action";
	});
	...
	function insertObject(id,name) {
		...
		for(var i=0,len=objectList.length;i<len;i++){
			if(objectList[i].id==id){
				objectList.splice(i, 1);noExistFlag=false;
			}
		}
		...
		var existName = "";
		for(var i=0,len=objectList.length;i<len;i++){
			existName += objectList[i].name + ",";
		}
		$j('#cheakObject').html(existName);
	}
</script>