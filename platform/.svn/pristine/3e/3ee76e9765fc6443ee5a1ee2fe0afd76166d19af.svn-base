...
// 				window.location.href=action;
			...
			// 新增信息 
			function doAdd() {
				var action = '<s:url namespace="/productionvisit/postpartumVisit" action="toAddPhPostpartumFollowRecord" includeParams="true"/>';
				var basicId = $j("#accountId",parent.document).val();
				window.location.href=action+'?accountId='+basicId
						+"&phPostpartumFollowRecord.followUpOpportunity="+<c:if test="${day==6}">0</c:if><c:if test="${day==40}">1</c:if>;
			}
			// 编辑信息 
			function doEdit() {
				var action = '<s:url namespace="/productionvisit/moreFollow" action="toAddPhPrenatalFollowRecordAfter" includeParams="true"/>';
			...
			<div class="wz_right">
				<div class="but01">
					<div class="pop_button_bar">
						<span><a href="javascript:doAdd();" class="pop_button_blue">产后${day}天随访</a></span>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="eXtremeTable">
						<s:if test="#request.list.size>0">
							<com.java.ec:table items="list" var="p" tableId="list" border="0"
								action="${pageContext.request.contextPath}/productionvisit/postpartumVisit/toList.action?accountId=${p.familyAccountInfo.id}"
