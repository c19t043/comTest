...
public class HttpClientUtils {
    ...
    public static String doPost(String url,Map<String,String> params){
		...
		try {
			req_Entity = new UrlEncodedFormEntity(paramList,"UTF-8");
    ...
    public static String doPost(String url,String content){
    	...
    	StringEntity req_Entity = new StringEntity(content,"UTF-8");
	...
	private static RequestConfig getConfig(){
		RequestConfig config = RequestConfig.custom().setConnectionRequestTimeout(150000) // 设置从connectManager获取Connection,超时时间，单位毫秒
				.setConnectTimeout(150000) // 设置连接超时时间，单位毫秒
				.setSocketTimeout(150000) // 请求获取数据的超时时间，单位毫秒
				.build();
		return config;
	}
