...
$(function(){
    $.ajax({
        ...
        success:function(result){
            ...
            if (result.mes == '请登录') {
                ...
                var list=result.doctorAddress;
                var doctor=result.doctorInfo;
                $('#addressSure>div:nth-child(2)').html(list.doctorProvince+'省'+list.doctorCity+list.doctorArea);
                $('#addressSure>div:nth-child(2)').attr('data-address-id',list.id);
                if(parseInt(doctor.serviceArea)==9999){
                    $('#addressSure>div:nth-child(4)').html('无限制(本市区内)');
                }else{
                    $('#addressSure>div:nth-child(4)').html(doctor.serviceArea);
                }
                $('#addressSure>div:nth-child(6)').html(doctor.comeMethod);
            }
        },
    ...
    $.ajax({
        ...
        success : function(result) {
            ...
            var dateNowStr=result.dateNowStr;
            var dateTimeStr=[9,12,14,18];
            var doctorProductList=result.doctorProductList;
            if(doctorProductList!=null){
                for(var ii= 0,len=doctorProductList.length;ii<len;ii++){
                    if($.inArray(doctorProductList[ii].serviceTimes,dateTimeStr)!=-1){
                        switch (doctorProductList[ii].serviceTimes) {
                            case 9:
                                $('td[zjtime='+doctorProductList[ii].serviceDate.substring(5)+']').eq(0).addClass('select');
                                break;
                            case 12:
                                $('td[zjtime='+doctorProductList[ii].serviceDate.substring(5)+']').eq(1).addClass('select');
                                break;
                            case 14:
                                $('td[zjtime='+doctorProductList[ii].serviceDate.substring(5)+']').eq(2).addClass('select');
                                break;
                            case 18:
                                $('td[zjtime='+doctorProductList[ii].serviceDate.substring(5)+']').eq(3).addClass('select');
                                break;
                        }
                    }
                }
    ...
    $(function(){
        $('.td_select_light_').click(function(){
            ...
            var arr_string='';
            var address_id = $('#addressSure>div:nth-child(2)').attr('data-address-id');
