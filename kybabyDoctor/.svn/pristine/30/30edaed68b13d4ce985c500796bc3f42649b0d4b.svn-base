	...
	$.ajax({
		...
		success:function(result){
			for(var j=0;j<result.majorList.length;j++){
				if(result.majorList[j].doctorType=="医生"){
			...
			for(var k=0;k<result.symptomTag.length;k++){
				$("#symptomTag").append("<li><span class='grey' onclick='changesymptom(this)' symptomId='"+result.symptomTag[k].id+"'>"+result.symptomTag[k].symptomName+"</span></li>");
			}
...
//	}
	...
	if(location.search.substring(1) == 'needInit'){			//修改时标签字符串重组---服务项目
		var arr_string='';
...
//	}
	...
	if($("#renzheng01").val()==""||$("#renzheng03").attr("data-hospital")==""
		||$("#renzheng08").val()==""||majorIds==null||majorIds==""||idCard==""||$("#renzheng09").val()==""){
		ale("请完善认证信息");
	}
	else if(location.search.substring(1) != 'needInit' && doctorHeadFileElem == ""){
		ale("请上传您的头像");
